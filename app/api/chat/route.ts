...
export async function POST(request: Request) {
  try {
    ...
    const [orders, products, insights, ppc] = await Promise.all([
      supabase.from('transactions').select('sku,title,true_profit,true_margin,sale_price,order_date,channel').order('order_date', { ascending: false }).limit(50),
      supabase.from('product_intelligence').select('*').limit(20),
      supabase.from('ai_insights').select('*').order('created_at', { ascending: false }).limit(5),
      supabase.from('ppc_keyword_performance').select('keyword_text,acos,spend,orders,clicks').order('spend', { ascending: false }).limit(20),
    ])
