import Link from 'next/link'
import { getSupabaseAdmin } from '../../lib/supabase-admin'
import { ListShell, StatusPill } from '../_components/ListShell'
import { STATUSES, theme } from '../_lib/theme'

export const dynamic = 'force-dynamic'

type Row = {
  id: string
  email: string
  company: string | null
  status: string
  partner_type: string | null
  country: string | null
  created_at: string
}

export default async function PartnersList({
  searchParams,
}: {
  searchParams: Promise<{ status?: string; q?: string }>
}) {
  const { status, q } = await searchParams
  const admin = getSupabaseAdmin()

  let query = admin
    .from('partner_applications')
    .select('id,email,company,status,partner_type,country,created_at')
    .order('created_at', { ascending: false })
    .limit(500)

  if (status) query = query.eq('status', status)
  if (q) query = query.ilike('email', `%${q}%`)

  const { data, error } = await query
  if (error) console.error('[admin:partners] list failed', error.message)
  const rows = (data ?? []) as Row[]

  return (
    <ListShell<Row>
      title="Partners"
      entityPath="/admin/partners"
      statuses={STATUSES.partners}
      currentStatus={status ?? null}
      search={q ?? ''}
      rows={rows}
      totalCount={rows.length}
      columns={[
        { header: 'Email',   render: r => <Link href={`/admin/partners/${r.id}`} style={{ color: theme.cobalt, textDecoration: 'none', fontWeight: 500 }}>{r.email}</Link> },
        { header: 'Company', render: r => r.company ?? '—' },
        { header: 'Type',    render: r => r.partner_type ?? '—' },
        { header: 'Country', render: r => r.country ?? '—' },
        { header: 'Status',  render: r => <StatusPill value={r.status} /> },
        { header: 'Created', render: r => new Date(r.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }), width: 120 },
      ]}
    />
  )
}
