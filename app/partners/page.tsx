...
const TIERS: Tier[] = [
  {
    name: 'Registered',
    tagline: 'You know one client who could use Fulcra.',
    ...
    needs: [
      ...
      'Fulcra overview certification (45 min, online).',
    ],
  },
  {
    name: 'Silver',
    ...
    needs: [
      'Three active accounts on Fulcra.',
  ...
  },
  {
    ...
    needs: [
      'Twenty-five active accounts on Fulcra.',
...
const TIMELINE = [
  ...
  { week: 'Week 1', title: 'Discovery',     desc: 'A 30-minute call to understand your client base and how Fulcra fits.' },
  { week: 'Week 2', title: 'Certification', desc: 'Two consultants complete the Fulcra certification. Async, ~3 hours.' },
...
const FAQ = [
  ...
  { q: 'Can I white-label Fulcra?',        a: 'Not yet. The Fulcra brand stays on the product. Your brand sits on implementation, support, and reporting.' },
    ...
    <header style={{ position: 'relative', overflow: 'hidden' }}>
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: '120px 32px 64px', position: 'relative', zIndex: 2 }}>
        ...
        <p style={{ fontSize: 18, lineHeight: 1.55, color: C.muted, maxWidth: 620, marginTop: 24 }}>
          Agencies, consultancies, and 3PLs already run their clients&rsquo; back offices on Fulcra. Refer one. Resell at scale. Co-build the operating layer for multichannel commerce.
        </p>
  ...
  return (
    <form onSubmit={onSubmit} style={{ marginTop: 32, display: 'grid', gap: 16, background: C.surface, border: `1px solid ${C.ruleSoft}`, borderRadius: 18, padding: 32, maxWidth: 720 }}>
      ...
      <div>
        ...
        <textarea id="about" name="about" rows={4} style={{ ...fieldInput, resize: 'vertical', fontFamily: 'inherit' }} placeholder="Specialism, channels you cover, why Fulcra fits." />
      </div>
...
function Nav({ active }: { active: string }) {
  return (
    <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(243,240,234,0.85)', backdropFilter: 'blur(10px)', borderBottom: `1px solid ${C.rule}`, padding: '0 32px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <Link href="/" style={{ fontSize: 16, fontWeight: 600, color: C.ink, textDecoration: 'none', letterSpacing: '-0.01em' }}>Fulcra</Link>
  ...
  return (
    <footer style={{ borderTop: `1px solid ${C.rule}`, background: C.bg }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '56px 32px 32px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 32 }}>
        <div>
          <Link href="/" style={{ fontSize: 18, fontWeight: 600, color: C.ink, textDecoration: 'none', letterSpacing: '-0.01em' }}>Fulcra</Link>
