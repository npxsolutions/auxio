...
export const metadata: Metadata = {
  title: 'Security · Palvento',
  description:
    'How Palvento handles your data — compliance posture, infrastructure, encryption, sub-processors, incident response, and your GDPR rights.',
}
...
const COMPLIANCE: Array<{ name: string; status: Status; detail: string }> = [
  ...
  { name: 'HIPAA',         status: 'na',       detail: 'Palvento is not a covered entity. Do not upload PHI.' },
    ...
    <div className={display.variable} style={{ background: C.bg, color: C.ink, fontFamily: sans, minHeight: '100vh' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 40, background: 'rgba(243,240,234,0.85)', backdropFilter: 'saturate(140%) blur(10px)', borderBottom: `1px solid ${C.rule}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', color: C.ink }}>
            ...
            <span style={{ fontFamily: serif, fontSize: 22, letterSpacing: '-0.015em' }}>Palvento</span>
          </Link>
      ...
      <header style={{ maxWidth: 1200, margin: '0 auto', padding: '96px 32px 72px', borderBottom: `1px solid ${C.rule}` }}>
        ...
        <h1 style={{ fontFamily: serif, fontWeight: 400, fontSize: 'clamp(44px, 7vw, 84px)', lineHeight: 1.04, letterSpacing: '-0.025em', margin: 0 }}>
          Security at <span style={{ fontStyle: 'italic', color: C.cobaltDk }}>Palvento</span>.
        </h1>
      ...
      <Section id="infra" label="02 / Infrastructure" title="Where your data lives." C={C}>
        <p style={paraStyle(C)}>
          Palvento runs on managed primitives from operators we trust — chosen so your data stays in the EU by default and never sits on a server we operate by hand.
        </p>
      ...
      <Section id="access" label="04 / Access control" title="Row-level by default." C={C}>
        <p style={paraStyle(C)}>
          Every public table in Palvento has Row Level Security enabled, and every policy is scoped by <code style={codeStyle(C)}>user_id = auth.uid()</code>. Our service role exists only inside server functions — it is never exposed to the browser, never committed to the repo, and rotated on role change.
        </p>
      ...
      <Section id="rights" label="08 / Your data" title="Your data is yours." C={C}>
        <p style={paraStyle(C)}>
          GDPR gives you the right to access, correct, export, and erase the personal data we hold about you. Palvento fulfils these rights in-app — no emails, no forms, no waiting.
        </p>
        <div style={{ marginTop: 28, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
