...
export const AMAZON_ERROR_MAP: MappedAmazonError[] = [
  ...
  { code: 'AUTH_TOKEN_INVALID', amazonCode: 'AccessDenied', match: /(token|access).*(invalid|expired|denied)/i, plainMessage: 'Your Amazon SP-API connection has expired or is unauthorized.', remediation: 'Reconnect Amazon in Meridia → Channels.' },
  { code: 'RATE_LIMITED', amazonCode: 'QuotaExceeded', match: /(quota|rate limit|429)/i, plainMessage: 'Amazon temporarily throttled this request.', remediation: 'Meridia will retry automatically; no action needed.' },
