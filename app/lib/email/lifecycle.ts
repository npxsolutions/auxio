...
const BASE_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://palvento-lkqv.vercel.app'
...
<tr><td>
  <div style="display:inline-flex;align-items:center;gap:8px;margin-bottom:28px;">
    ...
    <span style="font-size:14px;font-weight:600;color:#0b0f1a;letter-spacing:-0.01em;">Palvento</span>
  </div>
  ${inner}
  <div style="margin-top:36px;padding-top:20px;border-top:1px solid rgba(11,15,26,0.06);font-size:12px;color:#5a6171;line-height:1.6;">
    You're receiving this as part of your Palvento onboarding. <a href="${BASE_URL}/settings" style="color:#1d5fdb;text-decoration:none;">Manage email preferences</a>.
  </div>
</td></tr></table>
</td></tr></table>
</body></html>`
...
export function welcomeEmail(user: LifecycleUser, _ctx: LifecycleContext = {}): RenderedEmail {
  ...
  const subject = 'Welcome to Palvento.'
  const html = shell(`
    <h1 style="font-family:'Instrument Serif',Georgia,serif;font-size:34px;font-weight:400;letter-spacing:-0.02em;line-height:1.1;margin:0 0 20px;">Welcome to Palvento${name}.</h1>
    ${p('Thanks for signing up. Palvento is the operating layer for multichannel commerce — one place to see true profit across every marketplace you sell on.')}
  ...
  `)
  const text = `Welcome to Palvento${name}.

Thanks for signing up. Palvento is the operating layer for multichannel commerce — one place to see true profit across every marketplace you sell on.
...
export function day1NudgeEmail(user: LifecycleUser, _ctx: LifecycleContext = {}): RenderedEmail {
  ...
  const html = shell(`
    ...
    ${p('Pick whichever store has the most recent activity. Shopify, eBay, Amazon, Etsy — any of them work. Palvento does the rest: pulls orders, matches fees, surfaces margin.')}
...
Pick whichever store has the most recent activity. Shopify, eBay, Amazon, Etsy — any of them work. Palvento does the rest: pulls orders, matches fees, surfaces margin.
...
export function day3NudgeEmail(user: LifecycleUser, _ctx: LifecycleContext = {}): RenderedEmail {
  ...
  const html = shell(`
    ...
    ${p('First path: webhook registration. When a new sale happens, the marketplace pings Palvento in real time. Already set up for you.')}
...
First: webhook registration — when a new sale happens, the marketplace pings Palvento in real time. Already set up.
...
export function day7ActiveEmail(user: LifecycleUser, ctx: LifecycleContext = {}): RenderedEmail {
  ...
  const html = shell(`
    ...
    ${p('One — repricing rules. Set a floor, a ceiling, and a margin target. Palvento moves prices automatically against competitor velocity.')}
...
export function day7DormantEmail(user: LifecycleUser, _ctx: LifecycleContext = {}): RenderedEmail {
  ...
  const html = shell(`
    ...
    ${p('A week in and Palvento has not given you a profit number yet. That is almost always a setup thing we can fix in fifteen minutes on a call.')}
...
A week in and Palvento has not given you a profit number yet. That is almost always a setup thing we can fix in fifteen minutes on a call.
