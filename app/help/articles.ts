...
export const ARTICLES: Record<string, Article> = {
  ...
  'connect-channel': {
    ...
    summary: 'A five-minute walk-through of connecting any sales channel to Meridia.',
    lastUpdated: '2026-04-01',
    group: 'Getting started',
    body: `Meridia's core job is to collapse every marketplace, storefront, and ad account into one profit-aware view. Nothing works until a channel is connected.
...
## Run the OAuth flow
From the onboarding screen or Settings → Channels, click the marketplace you want. You'll be redirected to that platform's permissions page. Meridia only asks for the scopes it genuinely needs: read access to orders, listings, and fees; write access only where repricing or inventory sync is involved.

## Confirm the backfill window
On first connect, Meridia pulls the last 90 days of orders by default. You can change the window up to 24 months on the confirmation screen. A longer window produces richer historical charts but takes longer to complete.
...
## Next step
As soon as you see real numbers, head to the dashboard's Profit view. That's the single screen Meridia is built around — every other feature (repricing, forecasting, alerts) feeds from it.`,
  },

  'import-products': {
    title: 'Import products and costs',
    summary: 'How Meridia ingests your catalogue and where to add cost-of-goods data.',
    lastUpdated: '2026-03-28',
    group: 'Getting started',
    body: `Meridia does not force a separate product catalogue. Whatever your channels already have becomes the source of truth; Meridia sits on top.

## Automatic product sync
When you connect a channel, every listing is pulled into Meridia's unified product view. We match across channels using GTIN, MPN, and SKU (in that order). Identical products sold on Shopify and eBay appear as one entity, with each listing surfaced underneath.

## Where costs come from
Meridia reads three cost sources automatically: channel fees (from the marketplace's own APIs), payment processor fees (Stripe, Shopify Payments, PayPal), and shipping (where available on the order). Advertising cost is pulled from connected ad accounts.
...
— Per-supplier: if you've connected a purchase-order workflow, Meridia averages COGS across recent POs automatically.
— Bulk margin: as a last-resort placeholder, set a blanket margin assumption per category until real data arrives.

## Why this matters
Every profit figure you see downstream — dashboard, digest emails, forecasting — derives from these inputs. Getting COGS in on day one is the highest-leverage ten minutes you'll spend with Meridia.`,
  },
  ...
  'see-profit': {
    ...
    group: 'Getting started',
    body: `Getting to a real profit figure requires four data streams: orders, channel fees, payment fees, and cost of goods. Meridia handles the first three automatically; you supply COGS.
...
— You have a manual refund or discount Meridia can't see (add it to the order via the override button).`,
  },

  // ───────────────────────────────── Channels ────────────────────────────────
  'connect-shopify': {
    title: 'Connect Shopify',
    summary: 'Install the Meridia app and approve scopes — orders, payouts, and fulfilment.',
...
Running multiple Shopify stores under one brand. Connect each separately — each gets its own row in the channel list and contributes to the unified profit view.`,
  ...
  'connect-ebay': {
    title: 'Connect eBay',
    summary: 'OAuth, the 18-month order history quirk, and how Meridia handles eBay fee structures.',
...
In your WordPress admin, go to WooCommerce → Settings → Advanced → REST API → Add key. Choose Read/Write permission and copy the consumer key and secret. Paste them into Meridia's WooCommerce connect screen along with your store URL.

## Webhooks
Meridia registers webhooks automatically once credentials verify. If your WordPress site is behind strict Cloudflare rules or a staging-only firewall, you may need to allow-list Meridia's IP ranges (listed on the connect screen).

## The companion plugin (optional)
For sellers who want one-click install, we publish a free WordPress plugin that handles key creation, webhook registration, and health checks. Search "Meridia for WooCommerce" in the WP plugin directory.

## Currency and tax
WooCommerce stores multiple currencies per order if your store supports it. Meridia respects the order's recorded currency and converts to your reporting currency using the exchange rate at the order date. Tax is excluded from revenue calculations by default; toggle in Settings → Reporting.`,
  },
  ...
  'etsy-setup': {
    ...
    group: 'Channels',
    body: `Etsy moved to OAuth 2.0 with a hard-capped rate limit. Meridia handles both.
...
If you're on Etsy Payments (most sellers are), the full fee stack — transaction fees, payment processing, listing fees, offsite ads — is available from Etsy's ledger API. Meridia parses this automatically. For the small number of sellers still on PayPal-only, some fees need to be supplied manually.

## Offsite ads
Etsy auto-enrolls high-volume sellers in Offsite Ads, which charges 12–15% on attributed orders. Meridia surfaces these as a separate line in the fee breakdown so you can see at a glance how much offsite conversion is costing you.

## Rate limits
Etsy allows 10 requests per second per app. Meridia's sync queue respects this globally; on first connect, a large shop can take 60-90 minutes to fully populate.`,
  },
...
If you use BigCommerce's Channel Manager to push to Amazon or eBay, connect those platforms directly in Meridia as well — Meridia's attribution works better when talking to each marketplace as a first-class source.`,
  ...
  'see-true-profit': {
    ...
    summary: 'What Meridia includes (and excludes) in the profit figure shown everywhere.',
    lastUpdated: '2026-04-08',
    group: 'Operations',
    body: `"Profit" is a loaded word. Most tools show gross margin and call it profit. Meridia shows the number that actually lands in your bank.
...
Meridia flags any order with negative true profit in red. Usually it's one of three causes: a discount code that stacked below cost, a high-return SKU that tripped into loss after refund, or advertising attribution that landed heavily on a low-margin product.`,
  ...
  'set-first-repricing-rule': {
    ...
    summary: 'Start with a floor, ceiling, and margin target. Meridia handles the rest.',
...
If a rule is chasing a competitor below your floor, Meridia will hold at the floor and email you. That's a signal the market has changed and you need to re-examine the SKU's economics.`,
  ...
  'forecasting': {
    title: 'Forecasting',
    summary: 'How Meridia projects revenue and profit forward, and what drives the model.',
...
Your product launch calendar, your discount plans, or anything happening outside Meridia. For planning-grade forecasts, combine Meridia's baseline with your own lift assumptions.
...
Every forecast shows a 10/50/90 percentile band. A wide band means the model has low confidence — usually because your history is short, seasonal, or noisy.`,
  ...
  'bundles': {
    ...
    group: 'Operations',
    body: `Bundles in Meridia are logical groupings — they don't change how your channels list products, only how Meridia treats them internally.
...
## Bundles vs kits
A kit is a distinct product composed of multiple child products sold as one. Meridia supports kits separately — see the kits guide.`,
  },
  ...
  'costs': {
    ...
    group: 'Operations',
    body: `Costs are the one input Meridia can't read from a third-party API. Keeping them accurate is what separates a dashboard from a decision-support tool.
...
Occasionally you'll have an order with unusual economics — a wholesale discount, a returned-for-rework unit. Open the order and click "Override costs" to set order-specific values that don't disturb the SKU default.`,
  ...
  'api-overview': {
    title: 'API overview',
    summary: 'Meridia\'s read/write API, authentication, and rate limits.',
    lastUpdated: '2026-04-06',
    group: 'API & developer',
    body: `The Meridia API is a RESTful HTTPS service at api.auxio.app/v1. Everything you can do in the dashboard — read orders, pull profit reports, trigger repricing — is available programmatically.
...
All errors return a structured JSON body with code, message, and (where relevant) hint. Codes are stable across versions — safe to branch on.`,
  ...
  'webhooks': {
    ...
    group: 'API & developer',
    body: `Meridia emits webhooks for any state change that might matter to another system in your stack.
...
SDKs follow the API version they target (@auxio/node@1.x maps to /v1). Minor version bumps add endpoints; major version bumps require code changes.`,
  ...
  'billing': {
    ...
    group: 'Account',
    body: `Meridia bills monthly or annually via Stripe. Upgrades take effect immediately, prorated; downgrades apply at the next renewal.
...
Cancel any time from Settings → Billing → Cancel plan. Your account stays active until the end of the paid period, and your data is preserved for 60 days in case you come back.`,
  ...
  'team': {
    ...
    group: 'Account',
    body: `Bring your ops team into Meridia without giving them your login.
...
Operators and Viewers can be restricted to a subset of channels. Useful if an agency manages your eBay presence but not your Shopify.`,
  ...
  'security': {
    ...
    lastUpdated: '2026-04-01',
    group: 'Account',
    body: `Meridia is SOC 2 Type II in progress (final audit Q3 2026). Here's what's in place today.
