...
// Emit CSV — includes image_url column pointing at the PNG generated by
// scripts/generate-post-images.ts and served from the deployed site.
const rows = ['scheduled_date,scheduled_time,network,text,image_url,notes']
for (const p of posts) {
  const text = `${p.hook}\n\n${p.body}\n\n${p.hashtags}`
  const note = `Week ${p.week} Post ${p.n} — ${p.type} — Visual: ${p.visual}`
  const slug = `post-${String(p.n).padStart(2, '0')}`
  const imageUrl = `https://palvento.com/marketing/posts/${slug}.png`
  rows.push(`${dateFor(p.week, p.day)},${p.time},linkedin,${csvEscape(text)},${imageUrl},${csvEscape(note)}`)
}
